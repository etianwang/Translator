...
class PDFTranslatorUI(QtWidgets.QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle("Honsen PDF 翻译器")

        # 字体与背景
        font_path = resource_path("PDF_translator/站酷文艺体.ttf")
        font_id = QtGui.QFontDatabase.addApplicationFont(font_path)
        font_family = QtGui.QFontDatabase.applicationFontFamilies(font_id)[0]
        self.setFont(QtGui.QFont(font_family, 16))

        background = QtWidgets.QLabel(self)
        pixmap = QtGui.QPixmap(resource_path("PDF_translator/background.jpg")).scaled(self.size(), QtCore.Qt.AspectRatioMode.IgnoreAspectRatio)
        ...
        # 添加界面标题文本
        title_label = QtWidgets.QLabel("Honsen PDF 翻译器（联网版）")
        ...
        browse_btn = QtWidgets.QPushButton("浏览 PDF")
    ...
    def select_file(self):
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "选择 PDF 文件", "", "PDF 文件 (*.pdf)")
        if file_path:
            ...
            for _ in range(60):
                ...
                if status == 1:
                    ...
                    if url:
                        ...
                        out_name = f"{self.target_lang}_{base_name}_{time_str}.pdf"
...
def run_pdf_gui():
    try:
        ...
        window = PDFTranslatorUI()
        window.show()
        opened_windows["pdf"] = window

    except Exception as e:
        import traceback
        with open("pdf_crash.log", "w", encoding="utf-8") as f:
